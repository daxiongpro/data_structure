from PyQt5.QtCore import Qt, QSize, QPoint, pyqtProperty as Property, QPropertyAnimation, QEasingCurve
...
class QMaze(QWidget):
    ...
    def use_animation(self, end):
        ...
        self.animation.setEndValue(end.point)
        self.animation.setDuration(1000)
        self.animation.start()

    def is_success(self, end):
    ...
    def go(self,direction):
        endNode = self.playerNode.crawl(direction)
        self.use_animation(end=endNode)
        self.playerNode = endNode
        if self.is_success(endNode):
